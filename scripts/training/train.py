...
# This must be set before any torch.compile() operations
# Use a very short root path to minimize path length issues
_triton_cache = "C:\\tc"  # Very short path for Triton
_inductor_cache = "C:\\ic"  # Very short path for inductor
try:
    ...
    os.environ['TORCH_INDUCTOR_CACHE_DIR'] = _inductor_cache
except Exception:
    # Fallback if C:\ is not writable
    _cache_base = os.path.join(tempfile.gettempdir(), "c")
    os.makedirs(_cache_base, exist_ok=True)
    os.environ['TRITON_CACHE_DIR'] = _cache_base
    os.environ['TORCH_INDUCTOR_CACHE_DIR'] = _cache_base
